    ...
    <view class="container">
        ...
        <view class="coupon-list">
            <view class="coupon-item" v-for="(item, index) in couponList" :key="index">
                <view class="coupon">
                    <image src="/static/images/img_fuwuquan_blue_3x.png" class="coupon-img"></image>
                    <view class="left-tit-box">
                        <view class="tit">{{ item.goodsName }}</view>
                        <view class="term">剩余核销次数：{{ item.times }}</view>
                    </view>
                    <view class="right-detail" @tap="spread(index)">
                        <text class="detail-txt">查看详情</text>
                        <image src="/static/images/icon_next_3x.png" class="arrow"
                               :class="{ 'tui-rotate_90': item.spread }"></image>
                    </view>
                </view>
...
<!--                    <view class="code-num">{{ item.code }}</view>-->
                    ...
                    <view class="list-item">
                        <view class="item-tit">券名称</view>
                        <view class="item-con">{{ item.goodsName }}</view>
                    </view>
                    <view class="list-item">
                    ...
                    <view class="explain-text" v-for="(items, index) in item.useDescribe" :key="items">{{
                            items
                        }}
                    </view>
                </view>
        ...
        <view class="none" v-show="couponNum == 0">暂无可用核销券~</view>
    </view>
</template>

<script>
import {writeOffCode, writeOffCodeStatus} from "@/api";
...
export default {
    ...
    methods: {
        getData() {
            this.couponList = []
            writeOffCode({limit: 1000, page: 1}, 'get').then(res => {
                let {list} = res;
                list.forEach((item,key) => {
                    let it = {
                        ...
                        times: item.times,
                        spread: key === 0,
                        goodsName: `${item.goods.goodsName}`+'商品核销券',
                        suitStore: '全部',
                        useDescribe: ['1、可在任何适用商家内消费', '2、解释权归五衡益生堂所有']
                    }
                    this.couponList.push(it)
                })
                if(this.couponList.length > 0) {
                    this.couponQrCode(this.couponList[0].codeUrl, 'couponQrcode0');
                    this.chooseCode = this.couponList[0].code;
                    this.getStatus()
                }
            })
        },
        ...
        spread: function (index) {
            this.chooseCode = this.couponList[index].code;
            this.getStatus()
            let couponList = this.couponList;
            if (!couponList[index].spread) {
                setTimeout(() => {
                    this.couponQrCode(couponList[index].codeUrl, 'couponQrcode' + index);
                }, 60);
            }
